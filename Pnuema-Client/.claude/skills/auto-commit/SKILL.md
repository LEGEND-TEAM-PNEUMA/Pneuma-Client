...
- **커밋 메시지 type**: 영어로 작성 (`feat`, `fix`, `chore` 등)
- **커밋 메시지 subject (설명)**: 반드시 **한글**로 작성
- 예시: `feat: 카드 뒤집기 애니메이션 추가`
...
**작성 예시:**
```
feat: 카드 뒤집기 애니메이션 추가
fix: 인벤토리 슬롯 정렬 오류 수정
chore: ProjectSettings 빌드 타겟 변경
refactor: 게임 매니저 싱글톤 구조 개선
```
...
[1/N] feat: 카드 등록 폼 컴포넌트 추가
  - src/components/CardForm.tsx
  - src/api/card.ts
  - src/components/CardForm.test.tsx
 
[2/N] chore: ESLint 규칙 업데이트
  - .eslintrc.js
...
# 커밋 (이슈 있음)
git commit -m "<type>: <subject>" -m "Closes #N"
```
...
abc1234  feat: 카드 등록 폼 컴포넌트 추가
def5678  chore: ESLint 규칙 업데이트
